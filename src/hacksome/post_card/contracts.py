...
from hacksome.state import normalize_json, sha256_json, sha256_text
...
class BuildHandoffV1:
    """The exact five-field handoff already published by Creative C7."""
    ...
    def __post_init__(self) -> None:
        _string(self.source_run_id, label="handoff source_run_id")
        _string(self.idea_card_id, label="handoff idea_card_id")
        _sha256(self.idea_card_sha256, label="handoff idea_card_sha256")
        _markdown(self.challenge_markdown, label="handoff challenge_markdown")
        card = _markdown(
            self.initial_idea_card_markdown,
            label="handoff initial_idea_card_markdown",
        )
        if sha256_text(card) != self.idea_card_sha256:
            raise PostCardContractError(
                "handoff idea_card_sha256 does not match exact Idea Card bytes"
            )

    @classmethod
    def from_mapping(cls, value: Any) -> "BuildHandoffV1":
        raw = _exact_object(value, fields=HANDOFF_FIELDS, label="Build handoff")
        return cls(
            source_run_id=_string(
                raw["source_run_id"], label="handoff source_run_id"
            ),
            idea_card_id=_string(
                raw["idea_card_id"], label="handoff idea_card_id"
            ),
            idea_card_sha256=_sha256(
