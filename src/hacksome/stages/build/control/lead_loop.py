...
from hacksome.stages.build.control.agent_loop import (
    DEFAULT_MCP_CONFIG,
    DEFAULT_PROVIDER,
    _role_config,
    agent_loop,
...
Create one Goal:
python3 -m hacksome.stages.build.control.control_client create_goal \\
...
python3 -m hacksome.stages.build.control.control_client list_my_goals

Cancel a Goal only when your current judgment has genuinely withdrawn it:
python3 -m hacksome.stages.build.control.control_client cancel_goal \\
