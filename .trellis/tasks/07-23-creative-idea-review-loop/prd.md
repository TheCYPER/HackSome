...
## Creative 流程解剖器（2026-07-25）

真实 B 站赛题运行暴露出“流程按合同成功”和“人类真的理解最终 Idea”并不是同一
件事。Percy 需要一份中文、可交互、直接映射真实实现的说明页面，用来逐步检查
C0–C7 到底发生了什么，而不是只看一张压缩流程图。

页面必须：

- 把 C0、C1、C1W、C2、C3、C4、C5、C6A、C6B、C6 Human Review、C6C 与
  C7 分别作为可聚焦的流程节点；鼠标悬停、点击和键盘聚焦都能查看详情。
- 对每个节点展示：目标、输入、Controller/Agent/Web/Human 分工、fanout 或循环、
  产物、关卡/失败语义、Prompt 路径、Schema 路径和主要实现文件。
- 明确标出没有 Prompt 的 Controller-only / deterministic 步骤，避免把整个流程
  错解为“一串模型 Prompt”。
- 提供“架构合同”与“B 站真实运行”两个观察层；真实运行层至少显示：
  C1W 因高置信 signal 缺少第二来源而整体 invalidated、后续 safe palette 为空，
  7 个 Concept 经一次修订后只有 2 个通过 C4、C6B 只 shortlist 1 个，以及测试
  人审生成 1 张最终 Idea Card。
- 单独解释当前“30 秒能理解”只是由 Prompt、结构字段、自动 Red Team 与一次
  人类回执组成的代理信号；自动 Agent 并没有真正模拟一个无背景的人类看 30 秒。
- 页面内容使用中文，Prompt/Schema/代码路径保持可复制、可在仓库中追踪；不从
  在线服务加载运行所必需的字体、脚本或数据。
- 页面是只读说明工具，不读取或修改 run，不提交评审，也不改变 Creative route
  的任何合同、Prompt、Schema、状态机或最终产物。
- 支持桌面和窄屏、键盘、可见焦点和 `prefers-reduced-motion`。

验收时，Percy 应能在页面中回答三类问题：

1. “这个步骤为什么存在、实际调用了谁？”
2. “它产生了哪个文件，失败以后流程会停还是继续？”
3. “这次 B 站运行为什么没有用上网络梗，最终 Idea 又是在哪里被收敛成现在的？”
