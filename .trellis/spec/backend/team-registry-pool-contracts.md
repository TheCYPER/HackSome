...
本文描述 `src/hacksome/stages/build/control/handoff.py`、`team_registry.py`、
`team_pool.py` 和 `team_operator.py` 的 active 合同。单 Team 内部 Goal/Worker/
Verifier 仍由 `hackathon-team-runtime-contracts.md` 定义，operator 配置位于
`ops/build/`。
...
进程边界只允许 `python -m hacksome.stages.build.control.team_operator
...
以及 Creative v1/v2。Build control 不 import Approval 或 Ideation 私有类型；
新增 route contract 必须在两侧显式升级并补跨进程测试。
...
测试入口：`tests/stages/build/control/test_handoff.py`、
`tests/stages/build/control/test_team_registry.py`。
...
测试入口：`tests/stages/build/control/test_team_pool.py`、
`tests/stages/build/control/test_team_operator.py`。
