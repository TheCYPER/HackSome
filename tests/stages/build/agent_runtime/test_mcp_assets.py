...
from hacksome.stages.build import AGENTS_ROOT, ASSETS_ROOT
...
def test_agent_image_copies_cua_server_from_canonical_python_source():
    dockerfile = (REPO_ROOT / "ops" / "build" / "docker" / "agent.Dockerfile").read_text(
        encoding="utf-8"
    )

    assert (
        "COPY --chmod=0755 "
        "src/hacksome/stages/build/agent_runtime/cua_mcp.py "
        "/opt/foundagent/cua_mcp.py"
    ) in dockerfile
    assert not (REPO_ROOT / "ops" / "build" / "docker" / "cua_mcp.py").exists()
...
def test_playwright_uses_the_account_browser_wrapper(role):
    ...
    assert (
        playwright["command"]
        == "/opt/hacksome/hacksome/stages/build/assets/scripts/browser_mcp.sh"
    )
    assert "args" not in playwright
    wrapper = ASSETS_ROOT / "scripts" / "browser_mcp.sh"
